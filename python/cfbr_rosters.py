...
try:
    ...
    rosters = pd.DataFrame(columns = ['season', 'team', 'player', 'mod_name', 'href', 'class', 'pos'])
    hdr = True
...
for season in range(2009,2018):
    for index, row in team_hrefs.iterrows():
        if len(rosters.loc[(rosters['team'] == row['name']) & (rosters['season'] == season),]) < 1:
            team = pd.DataFrame(columns = ['season', 'team', 'player', 'mod_name', 'href', 'class', 'pos'])
            ...
            for player in players:
                ...
                team.loc[len(team),] = [season, row['name'], raw_name, mod_name, href, class_, pos]
            ...
            with open('csv\\rosters_cfbr.csv', 'ab') as f:
                team.to_csv(f, index = False, header = hdr)
            hdr = False
